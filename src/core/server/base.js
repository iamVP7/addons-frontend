...
export function getPageProps({ store, req, res, config }) {
  ...
  const statsFile = path.resolve(
    path.join(config.get('basePath'), 'dist/loadable-stats.json'),
  );
  const chunkExtractor = new ChunkExtractor({
    statsFile,
    entrypoints: ['amo'],
    outputPath: path.join(config.get('basePath'), 'dist/'),
  });
...
function baseServer(
  ...
  app.use(async (req, res, next) => {
    try {
      ...
      const props = {
        component: (
          <ChunkExtractorManager extractor={pageProps.extractor}>
            <Root
